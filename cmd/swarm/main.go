...
func registerBzzService(bzzconfig *bzzapi.Config, ctx *cli.Context, stack *node.Node) {
	...
	psql := ctx.GlobalBool("psql")
	psqlTableName := ctx.GlobalString("psqlTableName")
	...
	boot := func(ctx *node.ServiceContext) (node.Service, error) {
		...
		return swarm.NewSwarm(ctx, swapClient, ensClient, bzzconfig, bzzconfig.SwapEnabled, bzzconfig.SyncEnabled, bzzconfig.Cors, psql, psqlTableName)
	}
