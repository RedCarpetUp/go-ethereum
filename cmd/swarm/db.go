...
func dbExport(ctx *cli.Context) {
	...
	store, err := openDbStore(args[0], ctx.GlobalBool("psql"), ctx.GlobalString("psqlTableName"))
	if err != nil {
...
func openDbStore(path string, psql bool, psqlTableName string) (*storage.DbStore, error) {
	if _, err := os.Stat(filepath.Join(path, "CURRENT")); err != nil {
		return nil, fmt.Errorf("invalid chunkdb path: %s", err)
	}
	hash := storage.MakeHashFunc("SHA3")
	return storage.NewDbStore(path, hash, 10000000, 0, psql, psqlTableName)
}
